...
class SolveQ4:
    ...
    def __init__(self):
        ...
        self.maxv = 10
        self.angulos = [0,0,0]
        self.zero = Twist(Vector3(0,0,0), Vector3(0,0,0.0))
        self.frente = Twist(Vector3(v,0,0), Vector3(0,0,0.0))
        self.direita = Twist(Vector3(0,0,0), Vector3(0,0, -w ))
        self.esquerda = Twist(Vector3(0,0,0), Vector3(0,0,w))
        ...
        recebedor = rospy.Subscriber(topico_imagem, CompressedImage, self.rgb, queue_size=4, buff_size = 2**24)

        self.state = '0'
        self.w_vel = math.pi/20.0
        self.v_vel = 0.2
        self.state_machine = {
            '0': self.state_0,
            '1': self.state_1,
    ...
    def state_1(self):
        ...
        if (centro[0] < centro_robo[0]+limiar) and (centro[0] > centro_robo[0]-limiar):
            self.velocidade_saida.publish(self.frente)
